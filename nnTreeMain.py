...
class nnTreeInpOutMSE(mx.rnn.BaseRNNCell):

	def __init__(self, num_hidden, emb_dimension, prefix='',  params=None, args=[], inputName='', nodeNumber=-1, dropout=0.0):
		super(nnTreeInpOutMSE, self).__init__(prefix='nnTreeInpOutMSE_'+prefix+'_', params=params)
		self.args = args
		self.func = prefix
		...
		if self.func in nums or self.func=='NumberDecoder':
	...
	def __call__(self, inp, children):
		...
		# print 'func in call', self.func
		assert self.func != 'MSEnumber'
		...
		if children==None:
			...
			if len(children)==1:
				...
				if self.func == 'Equality':
					assert (not self.isNumeric() or (self.isNumeric() and self.args[0].func=='NumberDecoder' and self.args[1].func=='NumberDecoder'))
					# TODO: check dim=1?
					data = mx.symbol._internal._mul(children[0], children[1], dim=1)
					data = mx.symbol.sum(data)
				# elif self.func == 'Equality' and self.isNumeric():
				# 	if self.args[0].func == 'MSEnumber':
				# 		data = children[1]
				# 	elif self.args[1].func == 'MSEnumber':
				# 		data = children[0]
				# 	else:
				# 		print 'arg1:', self.args[0].func
				# 		print 'arg2:', self.args[0].func
				# 		raise AssertionError('one of the checks should be met')
					# mseInd = -1
					# for aInd, arg in self.args:
					# 	if arg.func!='MSEnumber':
					# 		mseInd = aInd
					# data = children[mseInd]
		...
		if self.func == 'Equality':
			# The purpose of the following fullyConnected layer is to add 
			# bias to the input dot product before feeding to sigmoid
			# if this bias is not added the output will always predict 1
			# (prob > 0.5)
			# the weight symbol for this fully connected layer is fixed.
			# this is handled through the main script by passing it as a 
			# fixed symbol to the bucketing module
			data = mx.sym.FullyConnected(data=data, weight=self._iW,
					                     bias=self._iB, num_hidden=1)
			state = mx.sym.reshape(data=data, shape=(1,), name='%sstate'%name)
			...
			i2h = mx.symbol.FullyConnected(data=data, weight=self._iW, bias=self._iB,
		                            ...
		                            name='%si2h'%name)
			act1 = mx.symbol.Activation(data=i2h, act_type="sigmoid", name='%sact1'%name)
			h2h = mx.symbol.FullyConnected(data=act1, weight=self._h2hW, bias=self._h2hB,
		                            num_hidden=self.num_hidden,
		                            name='%sh2h'%name)
			dp = mx.symbol.Dropout(data=h2h, p=self.dropout)
			state = mx.symbol.Activation(data=dp, act_type="sigmoid", name='%sstate'%name)
		elif self.func=='NumberDecoder':
			i2h = mx.symbol.FullyConnected(data=data, weight=self._iW, bias=self._iB,
		                            num_hidden=self.num_hidden,
		                            name='%si2h'%name)
			act1 = mx.symbol.Activation(data=i2h, act_type="sigmoid", name='%sact1'%name)
	...
	def unroll(self, dataNameDictionary):
		...
		if self.inputName=='':
			...
			if self.func in nums:
				...
				output_state = self(inp=inputDp, children=None)
	...
	def getDataNames(self, dataNames=[], nodeNumbers=[]):
		if len(self.args)==0 and self.func!='MSEnumber':
...
class lstmTreeInpOutMSE(mx.rnn.BaseRNNCell):

	def __init__(self, num_hidden, emb_dimension, prefix='',  params=None, args=[], inputName='', nodeNumber=-1, dropout=0.0):
		super(lstmTreeInpOutMSE, self).__init__(prefix='lstmTreeInpOutMSE_'+prefix+'_', params=params)
		self.args = args
		self.func = prefix
		self.num_hidden = num_hidden
		self.emb_dimension = emb_dimension
		...
		if self.func=='Variable' or self.func=='Const':
			self._iW = self._params.get('i2h_weight')
			self._iB = self._params.get('i2h_bias')
		elif self.func in nums or self.func=='NumberDecoder':
			self._iW = self._params.get('i2h_weight')
			self._h2hW = self._params.get('h2h_weight')
			self._iB = self._params.get('i2h_bias')
			self._h2hB = self._params.get('h2h_bias')
			...
			raise TypeError('uknown input function type {0}'.format(self.func))
	...
	def __call__(self, inp, children, memoryCh):
		...
		if children==None:
			...
			if self.func in nums:
				...
				raise TypeError('unknown leaf function type {0}'.fomrat(self.func))
			...
			if len(children) == 0:
				...
				if self.func == 'NumberDecoder':
					i2h = mx.symbol.FullyConnected(data=children[0], weight=self._iW, bias=self._iB,
			                                       num_hidden=self.num_hidden,
			                                       name='%si2h'%name)
					act1 = mx.symbol.Activation(data=i2h, act_type="sigmoid", name='%sact1'%name)
					h2h = mx.symbol.FullyConnected(data=act1, weight=self._h2hW, bias=self._h2hB,
				                                   num_hidden=1,
				                                   name='%sh2h'%name)
					next_state = mx.symbol.Activation(data=h2h, act_type="sigmoid", name='%sstate'%name)
					next_state = mx.symbol.Dropout(data=next_state, p=self.dropout)

					next_memory = mx.symbol.zeros(shape=(0,1), name='%sbegin_memory'%(name)) # consider feeding shape??
				elif self.func in functionOneInpSet:
					# memory = memoryCh[0]
					# state = children[0]
					...
					next_state = mx.symbol._internal._mul(out_gate, mx.symbol.Activation(next_memory, act_type="tanh"),
					                               name='%sstate'%name)
				elif self.func == 'Equality':
					assert self.isNumeric(), 'if equality has one child, then the equation is numeric'
					assert (self.args[0].func == 'MSEnumber' or self.args[1].func == 'MSEnumber')

					data = mx.sym.FullyConnected(data=children[0], weight=self._iW,
					                             bias=self._iB, num_hidden=1)
					next_state = mx.sym.reshape(data=data, shape=(1,), name='%sstate'%name)
					next_memory = None

				else:
					raise AssertionError('unexpected input function {0}'.format(self.func))
				...
				if self.func == 'Equality':
					...
					raise AssertionError('unexpected function {0}'.format(self.func))
	...
	def unroll(self, dataNameDictionary):
		...
		# if self.func == 'Equality':
		# 	print 'num children:', len(self.args)
		# 	print 'child1:', self.args[0].func
		# 	print 'child2:', self.args[1].func
	...
	def isNumeric(self):
		...
		if self.func == 'Number' or self.func == 'MSEnumber':
			return True
		else:
			return (False or flag)
